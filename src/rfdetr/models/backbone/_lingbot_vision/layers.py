...
Contains the building blocks used by the backbone: RoPE position embedding, register/storage tokens support,
LayerNorm/RMSNorm variants, MLP and SwiGLU FFNs, and SDPA attention.
"""
...
class RopePositionEmbedding(nn.Module):
    ...
    Height and width use independent rotation frequencies (no coordinate mixing) and there are no learnable parameters.
    ``periods`` is a persistent buffer so that checkpoints carry it. Patch coordinates are normalized to [-1, 1]; in
    training mode the optional shift/jitter/rescale settings randomly perturb the coordinates as augmentation.
    ``forward(H=..., W=...)`` returns ``(sin, cos)`` tables of shape ``[H*W, D_head]``.
    """
...
class LinearKMaskedBias(nn.Linear):
    ...
    The bias is multiplied elementwise by a persistent ``bias_mask`` buffer before the linear op; with ``mask_k_bias``
    the mask zeroes the K third of the bias so keys carry no bias while Q and V biases pass through. The buffer starts
    as NaN so an unfilled mask fails loudly; ``init_weights()`` sets it (ones, with the K third zeroed) and a loaded
    checkpoint may overwrite it.
    """
...
class SelfAttentionBlock(nn.Module):
    ...
    def _forward_list(self, x_list: List[Tensor], rope_list=None) -> List[Tensor]:
        """Run the block over a list of token tensors, concatenating tokens across the list so norms and FFNs execute as
        single batched ops.
