...
The backend is selected once per process, either via :func:`set_backend` or the ``LINGBOT_VISION_ATTN_BACKEND``
environment variable (read on first attention call). Supported values:

sdpa          torch SDPA with automatic kernel dispatch (default) sdpa-cudnn    torch SDPA forced to the cuDNN kernel
sdpa-flash    torch SDPA forced to the flash kernel sdpa-mem_eff  torch SDPA forced to the memory-efficient kernel

Forcing a specific kernel uses ``torch.nn.attention.sdpa_kernel`` and therefore requires torch >= 2.3 (``sdpa-cudnn``
needs torch >= 2.4, which added ``SDPBackend.CUDNN_ATTENTION``); the default ``sdpa`` backend works on older versions.
"""
...
def attention(q: Tensor, k: Tensor, v: Tensor, layout: str = "bnhd") -> Tensor:
    ...
    layout "bnhd": q/k/v are [B, N, heads, head_dim]. layout "bhnd": [B, heads, N, head_dim] (SDPA-native). Returns the
